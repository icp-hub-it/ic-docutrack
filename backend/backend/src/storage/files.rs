...
pub use self::create_state::{ File, FileId, ChunkId, FileMetadata, FileContent};

use crate::storage::memory::{MEMORY_MANAGER, FILE_COUNT_MEMORY_ID,OWNED_FILES_MEMORY_ID, FILE_DATA_MEMORY_ID, FILE_ALIAS_INDEX_MEMORY_ID, FILE_SHARES_MEMORY_ID, FILE_CONTENTS_MEMORY_ID,SHARED_KEYS_MEMORY_ID,ALIAS_GENERATOR_MEMORY_ID};
...
thread_local! {
  ...
  fn with_owned_files<T, F>(f: F) -> Vec<u64>
  where
      F: FnOnce(FileId) -> T,
  {
  ...
  where
      F: FnOnce(&StableBTreeMap<StorablePrincipal, StorableFileIdVec, VirtualMemory<DefaultMemoryImpl>>) -> T,
  {
  ...
  where
      F: FnOnce(Vec<u8>) -> T,
  {
      FILE_CONTENTS_STORAGE.with_borrow(|file_contents| file_contents.get(&(file_id.clone(), chunk_id.clone())).map(f))
  }
    ...
    where
        F: FnOnce(Vec<u8>) -> T,
    {
        SHARED_KEYS_STORAGE.with_borrow(|shared_keys| shared_keys.get(&StorablePrincipal::from(*principal)).map(f))
    } 
    ...
    impl FileSharesStorage {
        ...
        pub fn set_file_shares(principal: &StorablePrincipal, file_ids: StorableFileIdVec) {
            FILE_SHARES_STORAGE.with_borrow_mut(|file_shares| {
                file_shares.insert(principal.clone(), file_ids);
            });
        }
...
//     alias_generator: AliasGenerator,
    ...
    mod test {
        use super::*;
        use self::create_state::{File, FileMetadata, FileContent};
        use did::StorablePrincipal;
        ...
        fn test_file_contents_storage() {
            ...
            assert_eq!(FileContentsStorage::get_file_contents(&file_id, &chunk_id), Some(contents));

            FileContentsStorage::remove_file_contents(&file_id, &chunk_id);
            assert_eq!(FileContentsStorage::get_file_contents(&file_id, &chunk_id), None);
        }
