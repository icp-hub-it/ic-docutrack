...
fn main() {
    ...
    let orbit_station_did = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join(ORBIT_STATION_DID_PATH);

    // make parent dir
    if let Some(parent) = orbit_station_did.parent() {
    ...
    fs::write(&orbit_station_did, file_content).expect("Failed to write generated Rust file");
}
