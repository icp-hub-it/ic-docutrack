...
use did::backend::{PublicFileMetadata, FileStatus, UploadFileRequest,UploadFileAtomicRequest,UploadFileContinueRequest, ENCRYPTION_KEY_SIZE};


use integration_tests::{BackendClient, PocketIcTestEnv,{actor::{bob, alice}}};
...
async fn test_should_get_alias_info() {
    ...
    assert_eq!(
      client.
        get_alias_info("not-an-alias".to_string(), external_user).await,
        Err(did::backend::GetAliasInfoError::NotFound)
    );
    assert_eq!(
        alias_info.clone().unwrap().file_name,
        request_name
    );
    assert_eq!(
        alias_info.unwrap().file_id,
        1
    );
...
async fn test_should_upload_file_atomic() {
    ...
    let owner = bob();
    let request_name = "test.txt".to_string();
    let file_id = client.upload_file_atomic(
...
async fn test_should_download_file() {
    ...
    let download_response = client.download_file(file_id.clone(), 2, owner).await;
    assert_eq!(download_response, did::backend::FileDownloadResponse::NotUploadedFile);
    client.upload_file_continue(
        UploadFileContinueRequest {
            file_id: file_id.clone(),
            chunk_id: 3,
