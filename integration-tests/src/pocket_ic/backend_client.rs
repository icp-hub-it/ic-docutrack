...
use did::backend::{
    AliasInfo, FileDownloadResponse, FileSharingResponse, GetAliasInfoError,
    PublicFileMetadata, UploadFileAtomicRequest, UploadFileContinueRequest, UploadFileError,
    UploadFileRequest,OwnerKey
};
...
impl BackendClient<'_> {
    pub async fn public_key(&self,caller: Principal) -> PublicKey {
        self.pic
            .query::<PublicKey>(self.pic.backend(), caller,
            "public_key", vec![])
            .await
    ...
    pub async fn set_public_key(&self, public_key: PublicKey) {
        ...
        self.pic
            .update::<()>(self.pic.backend(), bob(), "set_public_key", payload)
            .await
    ...
    pub async fn get_shared_files(&self, caller: Principal) -> Vec<PublicFileMetadata> {
        let payload = candid::encode_args(()).unwrap();
        self.pic
            .query::<Vec<PublicFileMetadata>>(self.pic.backend(), caller, "get_shared_files", payload)
            .await
            .expect("Failed to get shared files")
    }
    pub async fn get_alias_info(&self, alias: String, caller: Principal) -> Result<AliasInfo, GetAliasInfoError> {
        let payload = candid::encode_args((alias,)).unwrap();
        self.pic
            .query::<Result<AliasInfo, GetAliasInfoError>>(self.pic.backend(), caller, "get_alias_info", payload)
            .await
            .expect("Failed to get alias info")
    }
    pub async fn upload_file(&self, request: UploadFileRequest, caller: Principal) -> Result<(), UploadFileError> {
        let payload = candid::encode_args((request,)).unwrap();
        self.pic
            .update::<Result<(), UploadFileError>>(self.pic.backend(), 
            caller, "upload_file", payload)
            .await
            .expect("Failed to upload file")
            
    }
    pub async fn upload_file_atomic(&self, request: UploadFileAtomicRequest, caller: Principal) -> u64 {
        let payload = candid::encode_args((request,)).unwrap();
        self.pic
    ...
    pub async fn upload_file_continue(&self, request: UploadFileContinueRequest, caller: Principal) {
        let payload = candid::encode_args((request,)).unwrap();
        self.pic
    ...
    pub async fn download_file(&self, file_id: u64, chunk_id: u64, caller: Principal) -> FileDownloadResponse {
    ...
    pub async fn share_file(&self, file_id: u64, user_id: Principal, file_key_encrypted_for_user: OwnerKey, caller: Principal) -> FileSharingResponse {
    ...
    pub async fn share_file_with_users(&self, user_id: Vec<Principal>, file_id: u64, file_key_encrypted_for_user: Vec<OwnerKey>, caller: Principal) -> () {
        let payload = candid::encode_args((user_id, file_id, file_key_encrypted_for_user)).unwrap();
        self.pic
